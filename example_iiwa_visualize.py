...
# while visualizing it in DrakeVisualizer
#
# STEP1: start the kuka_driver
# STEP2: start drake-visualizer and or meshcat-server, which is located in
...
class EndEffectorTeleop(LeafSystem):
    def __init__(self, planar=False):
        ...
        self.y = tk.Scale(self.window, from_=-0.8, to=0.3,
                          ...
                          orient=tk.HORIZONTAL)
        if not planar:
            self.y.pack()#set the initial value for the differntial Ik integrator
        self.y.set(0)
